...
export default function ModifyProduct({ history }) {
  ...
  useEffect(() => {
    api
      .get("/transports", { headers: { userToken: userToken } })
      .then((result) => {
    ...
    api
      .get("/conditions", { headers: { userToken: userToken } })
      .then((result) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (name === undefined) name = product.data.name;
    if (description === undefined) description = product.data.description;
    if (conditionId === undefined) conditionId = product.data.condition._id;
    if (transportId === undefined) transportId = product.data.transport._id;
  ...
  const deleteProduct = async () => {
    try {
      await api.delete(`/matchs/delete/${productId}`, {
        headers: { userToken: userToken },
      });
          ...
          <FormControl className="productForm">
            ...
            >
              {conditions.data.map((condition) => (
                <option value={condition._id} key={condition._id}>
                  {condition.name}
                </option>
              ))}
            </NativeSelect>
          ...
          <FormControl className="productForm">
            ...
            >
              {transports.data.map((transport) => (
                <option value={transport._id} key={transport._id}>
                  {transport.name}
                </option>
              ))}
            </NativeSelect>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              Êtes-vous sûr de vouloir supprimer ce produit ? Les potentiels
              'match' en cours seront annulés par la même occasion.
            </DialogContentText>
          </DialogContent>
          <DialogActions>
