...
export default function Proposal({ history }) {
  ...
  const handleSubmit = async (event) => {
    ...
    let product;
    try {
      product = await api.get(`/product/${productId}`, {
      ...
      if (state[checked]) {
        try {
          await api.post(
            `/match/add`,
            {
              owner: product.data.user._id,
              productOwner: productId,
              productConsignee: productsUser.data[checked],
            },
            { headers: { userToken: userToken } }
          );
        } catch (error) {
  ...
  const getProducts = () => {
    ...
    if (productsUser.data.length) {
              ...
              <FormControlLabel
                ...
                label={
                  <img
                    ...
                    className="productImg proposalProduct"
                    alt={`product${i}`}
                    draggable="false"
                  ></img>
                }
      ...
      products.push(
        <h1 key="noProducts">
          Vous n'avez pas encore de produit à échanger, veuillez en ajouter !
        </h1>
      );
    }
